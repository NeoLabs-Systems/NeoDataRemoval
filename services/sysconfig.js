...
function _readRow(db, key) {
  try {
    return db
      .prepare(
        "SELECT value, is_encrypted, updated_at FROM system_settings WHERE key = ?",
      )
      .get(key);
  } catch {
    return null;
...
function getPublicConfig() {
  ...
  if (db) {
    try {
      const rows = db
        .prepare("SELECT key, is_encrypted, updated_at FROM system_settings")
        .all();
  ...
  for (const key of ALL_KEYS) {
    ...
    result[key] = {
      ...
      value: isSens ? (dbRow || hasEnv ? "••••••••" : null) : get(key),
    };
  }
