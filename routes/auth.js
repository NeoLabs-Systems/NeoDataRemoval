...
const authLimiter = rateLimit({ windowMs: 15 * 60 * 1000, max: 10, legacyHeaders: false });

/* GET /api/auth/can-register */
router.get('/can-register', (req, res) => {
  if (process.env.ADMIN_REGISTRATION !== 'closed') return res.json({ allowed: true });
  const db  = getDb();
  const any = db.prepare('SELECT id FROM users LIMIT 1').get();
  res.json({ allowed: !any });
});
...
router.post('/register', authLimiter, async (req, res) => {
  ...
  if (process.env.ADMIN_REGISTRATION === 'closed') {
    const db = getDb();
    const any = db.prepare('SELECT id FROM users LIMIT 1').get();
    if (any) return res.status(403).json({ error: 'Registration is closed' });
  }
  ...
  ).run(username.trim(), normalEmail, hash, role);
...
/* GET /api/auth/2fa/generate — create secret, store it (not yet enabled), return QR */
...
router.post('/2fa/disable', requireAuth, async (req, res) => {
  ...
  const db   = getDb();
  const user = db.prepare('SELECT password_hash, totp_secret, totp_enabled FROM users WHERE id = ?').get(req.user.id);
